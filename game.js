// =============================================
// A MAZE ZING! - Complete Working Version
// Large Maze + Procedural Levels + Shield + Shooting
// =============================================
...
function draw() {
    ...
    // AI
    if (aiEnabled) {
        ctx.shadowBlur = Date.now() < ai.stunnedUntil ? 0 : 25;
        ctx.shadowColor = '#ff0088';
        ctx.fillStyle = Date.now() < ai.stunnedUntil ? '#ff88aa' : '#ff0088';
...
function updatePlayerProjectiles() {
    for (let i = playerProjectiles.length - 1; i >= 0; i--) {
        ...
        if (aiEnabled && Math.floor(ai.x) === gx && Math.floor(ai.y) === gy) {
            ai.stunnedUntil = Date.now() + 4500;
            playerProjectiles.splice(i, 1);
            playNote(150, 500, 'sawtooth', 0.5);
            continue;
        }
